...
public class MainActivity2 extends AppCompatActivity implements View.OnCreateContextMenuListener {
    ...
    public boolean onContextItemSelected(MenuItem item) {
        // בדיקה אם ה-MenuInfo זמין
        ContextMenu.ContextMenuInfo menuInfo = item.getMenuInfo();

        // וידוא שהפעולה מתבצעת על רכיב תקין
        int position = -1; // ערך ברירת מחדל
        if (menuInfo instanceof AdapterView.AdapterContextMenuInfo) {
            AdapterView.AdapterContextMenuInfo info = (AdapterView.AdapterContextMenuInfo) menuInfo;
            position = info.position;
        }

        // ודאי ש-position תקין
        if (position == -1 || position >= arr2.length) {
            tV.setText("שגיאה: מיקום לא תקין");
            return true; // להפסיק את הטיפול
        }

        // פעולה בהתאם לאפשרות שנבחרה
        String action = item.getTitle().toString();
        if (action.equals("מקומו של האיבר בסדרה")) {
            tV.setText("מיקום האיבר בסדרה: " + (position + 1));
        } else if (action.equals("סכום בין האיבר הראשון ל-איבר שנבחר")) {
            double sum = 0;
